...
static int ledGpsFlashCounter = 0;
static int ledMavlinkFlashCounter = 0;
...
static DEVICE_MODE device_mode;
...
//        Serial.write('4');
	while (Serial.available() > 0) {
  		...
  		char inChar = Serial.read(); // Read a character
               ...
               if(searchIndex == 0 && inChar == 'G'){
                  searchIndex = 1;
        //Serial.write('t');
          //        searchIndex = 0;
            //      return true;		
} else             if(searchIndex == 1 && inChar == 'N'){
                  searchIndex = 2;
		} else                 if(searchIndex == 2 && inChar == 'G'){
                  searchIndex = 3;
		} else                 if(searchIndex == 3 && inChar == 'S'){
                  searchIndex = 4;
		} else                 if(searchIndex == 4 && inChar == 'A'){
                  Serial.write('t');
                  searchIndex = 0;
                  return true;
		} else 
...
//Serial.print(inChar);
		...
		if (index33 < 55) { // One less than the size of the array



      			inData[index33] = inChar; // Store it
			index33++; // Increment where to write next
			inData[index33] = '\0'; // Null terminate the string
		}
	}
	// GPRMC - rzadko
char *ret = strcasestr(inData, "GNGSA");
//  Serial.println("Ret=" + *ret);

	if (index33 > 5 && ret != 0) {
//	if (index33 > 5 && strstr(inData, "GNGSA")) {
		Serial.write('T');
		return true;
	}
	Serial.write('n');
...
//////////////////////////////////////////////////////////////////////////////////////////////////////// GPS_SERIAL
	...
	if (GPS_SERIAL == device_mode) {
              ...
              //Serial.write('g');
		if (millis() - ledGpsActivityTimer > 4000) {
			ledGpsActivityTimer = millis();
			ledGpsMode = OFF;
...
//if(true){
		...
		if (readAndParse()) {
			ledGpsActivityTimer = millis();
                        Serial.write("G");
//      giveTinyGPS()->hdop();    
			if (TinyGPS::GPS_INVALID_SATELLITES == tiny.satellites())
				Serial.write("parseOK   NOFIX");
			else
				Serial.write("parseOK  SATS: " + tiny.satellites());

			unsigned long age;
			float flat = 0.0, flon = 0.0;
			tiny.f_get_position(&flat, &flon, &age);
			Serial.print("LAT=");
...
//				beacon_send_number(flon, 2, 5, 3);
			...
			Serial.print(" PREC=");
			Serial.print(
					tiny.hdop() == TinyGPS::GPS_INVALID_HDOP ?
							0 : tiny.hdop());
