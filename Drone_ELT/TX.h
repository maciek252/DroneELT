...
uint16_t getChannel(uint8_t ch) {
	...
	if (ch < 16) {
		uint16_t v;
		cli();
		// disable interrupts when copying servo positions, to avoid race on 2 byte variable written by ISR
		v = PPM[ch];
		sei();
		return v;
	} else {
...
//      giveTinyGPS()->hdop();    
		...
		Serial.print(
				giveTinyGPS()->satellites() == TinyGPS::GPS_INVALID_SATELLITES ?
						0 : giveTinyGPS()->satellites());
		Serial.print(" PREC=");
		Serial.print(
				giveTinyGPS()->hdop() == TinyGPS::GPS_INVALID_HDOP ?
